...
#Using the function str() we see that the selected varibales in covidvax2 are chr, int, chr, num, num 
#We may need to convert the dates to numeric using the as.numeric() function to determine and compare vaccination rate. All other column types should be fine as is. 
#Converting NA cells to 0 we allow ourselves to manipulate and describe entire numeric columns
...
covidvax2 %>% group_by(zip_code) %>% summarise(fullyvax_range = range(fully_vaccinated), partiallyvax_range = range(partially_vaccinated))

covidvax2 %>% summarise(total_pop_range = range(total_pop_over_12), total_pop_median = median(total_pop_over_12))
...
#Using the function str() we see that the selected varibales in countydem2 are chr, int, int, int, int 
...
countydem2 %>% summarise(renters_mean = mean(renters), owners_mean = mean(owners), vacant_mean = mean(vacant_housing), total_mean = mean(total_housing_units))

countydem2 %>% summarise(renters_range = range(renters), owners_range = range(owners), vacant_range = range(vacant_housing), total_range = range(total_housing_units))
```
...
countydem3 <- countydem2 %>%
  ...
  mutate(renter_housing_ratio = (total_housing_units - vacant_housing)/renters) %>%
  mutate(owner_housing_ratio = (total_housing_units - vacant_housing)/owners) %>%
...
countydem4 <- countydem3 %>%
  summarise(total_in_use_housing_range = summary(total_in_use_housing),
            renter_housing_ratio_range = summary(renter_housing_ratio),
            owner_housing_ratio_range = summary(owner_housing_ratio),
...
countydem_kable2 <-kable(countydem5,longtable=T,booktabs=T, 
                         ...
                         caption = "Total Number of Renters & Owners in CA in 2021")
countydem_kable2
```
